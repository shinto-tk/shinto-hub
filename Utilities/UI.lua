...
or InsertService:LoadLocalAsset("rbxassetid://10445019386")
...
local function GetConfigs()
    if not isfolder("Shinto") then makefolder("Shinto") end
    if not isfolder("Shinto\\Configs") then makefolder("Shinto\\Configs") end
    if not isfile("Shinto\\DefaultConfig.txt") then writefile("Shinto\\DefaultConfig.txt","") end

    local Configs = {}
    for Index,File in pairs(listfiles("Shinto\\Configs") or {}) do
        File = File:gsub("Shinto\\Configs\\","")
        File = File:gsub(".json","")
        Configs[Index] = File
    end
    return Configs
...
local function ConfigsToList()
    ...
    for Index,File in pairs(listfiles("Shinto\\Configs") or {}) do
        File = File:gsub("Shinto\\Configs\\","")
        File = File:gsub(".json","")
        local DefaultConfig = readfile("Shinto\\DefaultConfig.txt")
...
local function InitScreen()
    ...
    if Debug then
        ...
        Shinto.Utilities.Misc:HideObject(ScreenAsset)
    end
...
local function InitWindow(ScreenAsset,Window)
    ...
    function Window:SaveConfig(Name)
        ...
        if table.find(GetConfigs(),Name) then
            Config = HttpService:JSONDecode(readfile("Shinto\\Configs\\"..Name..".json"))
        end
        for Index,Element in pairs(Window.Elements) do
        ...
        writefile("Shinto\\Configs\\"..Name..".json",HttpService:JSONEncode(Config))
    end
    function Window:LoadConfig(Name)
        if table.find(GetConfigs(),Name) then
            local DecodedJSON = HttpService:JSONDecode(readfile("Shinto\\Configs\\"..Name..".json"))
            for Index,Element in pairs(Window.Elements) do
    ...
    function Window:DeleteConfig(Name)
        if table.find(GetConfigs(),Name) then
            delfile("Shinto\\Configs\\"..Name..".json")
        end
    end
            ...
            local ConfigSection = Tab:Section({Name = "Configs",Side = Side}) do
                ...
                ConfigSection:Button({Name = "Set",Callback = function()
                    if ConfigDropdown.Value and ConfigDropdown.Value[1] then
                        ...
                        writefile("Shinto\\DefaultConfig.txt",DefaultConfig)
                        ConfigDivider:SetText(
                        "Default Config\n<font color=\"rgb(189,189,189)\">[ "..DefaultConfig.." ]</font>")
                    end
                end})
                ConfigSection:Button({Name = "Clear",Callback = function()
                    writefile("Shinto\\DefaultConfig.txt","")
