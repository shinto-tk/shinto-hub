...
local Window = Shinto.Utilities.UI:Window({
    Name = "Shinto Hub — "..Shinto.Game,
    ...
    local VisualsTab = Window:Tab({Name = "Visuals"}) do
        local GlobalSection = VisualsTab:Section({Name = "Global",Side = "Left"}) do
            ...
            GlobalSection:Slider({Name = "Distance",Flag = "ESP/Player/Distance",Min = 25,Max = 9999,Value = 250,Unit = "meters"})
        end
        ...
        local LightingSection = VisualsTab:Section({Name = "Lighting",Side = "Right"}) do
            ...
            Callback = function(Bool) if Bool then return end
                for Property,Value in pairs(Shinto.Utilities.Misc.DefaultLighting) do
    ...
    local SettingsTab = Window:Tab({Name = "Settings"}) do
        ...
        Callback = Shinto.Utilities.Misc.ReJoin})
        SettingsTab:Button({Name = "Server Hop",Side = "Left",
        Callback = Shinto.Utilities.Misc.ServerHop})
        SettingsTab:Button({Name = "Join Discord Server",Side = "Left",
        Callback = Shinto.Utilities.Misc.JoinDiscord})
        ...
        local CreditsSection = SettingsTab:Section({Name = "Credits",Side = "Right"}) do
            CreditsSection:Label({Text = "This script was made by not alex#0001"})
            ...
            CreditsSection:Label({Text = "Thanks to AlexR32 for Bracket V3.2"})
        end
    end
end
...
Shinto.Utilities.Misc:SetupWatermark(Window)
Shinto.Utilities.Misc:SetupLighting(Window.Flags)
Shinto.Utilities.Drawing:SetupCursor(Window.Flags)

Shinto.Utilities.Drawing:FOVCircle("Aimbot",Window.Flags)
Shinto.Utilities.Drawing:FOVCircle("Trigger",Window.Flags)
Shinto.Utilities.Drawing:FOVCircle("SilentAim",Window.Flags)
...
Shinto.Utilities.Misc:NewThreadLoop(1,function()
...
for Index,Item in pairs(Loot:GetDescendants()) do
    ...
    if Item:IsA("Model") and ItemData then --print(ItemData.Parent.Name)
        Shinto.Utilities.Drawing:ItemESP(
...
Randoms.ChildAdded:Connect(function(Place)
    if table.find(Places,Place.Name) then --print(Place.Name)
        ...
        if Window.Flags["AR2/ESP/RandomPlaces/Enabled"] then
            Shinto.Utilities.UI:Notification2({
...
Loot.DescendantRemoving:Connect(function(Item)
    if Item:IsA("Model") then
        Shinto.Utilities.Drawing:RemoveESP(Item.Parent)
    end
end)
Randoms.ChildRemoved:Connect(function(Place)
    Shinto.Utilities.Drawing:RemoveESP(Place)
end)
Vehicles.ChildRemoved:Connect(function(Vehicle)
    Shinto.Utilities.Drawing:RemoveESP(Vehicle)
end)
Zombies.ChildRemoved:Connect(function(Zombie)
    Shinto.Utilities.Drawing:RemoveESP(Zombie)
end)
...
for Index,Player in pairs(PlayerService:GetPlayers()) do
    ...
    Shinto.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
end
PlayerService.PlayerAdded:Connect(function(Player)
    Shinto.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
end)
PlayerService.PlayerRemoving:Connect(function(Player)
    Shinto.Utilities.Drawing:RemoveESP(Player)
end)
